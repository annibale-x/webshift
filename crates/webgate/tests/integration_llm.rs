...
//! Run with: `cargo test -p webgate --features "backends llm" -- --ignored`
...
async fn llm_summarization_live() {
    ...
    let result = webgate::query(&["rust async runtime"], &config)
        .await
        .expect("query failed");
...
async fn llm_full_pipeline_live() {
    ...
    let result = webgate::query(&["benefits of functional programming"], &config)
        .await
        .expect("query failed");
